...
# It was, and that made it invisible to every signed-in user while looking correct
# in the file and in review: on a fresh page load session_state is empty, so the
# logo rendered, then render_login_or_register() read the auth cookie and called
# st.rerun() (auth.py) - and on that second run the condition was False, the
# element was never emitted, and Streamlit removed it. The logo appeared for about
# a second on every refresh and then vanished, which reads exactly like a CSS or
# z-index fault and is not one. "Public landing page" means the product's own
# front page rather than the admin console; it does not mean "logged-out only".
st.markdown(
