...
# The req2prod logo, pinned to the top-right corner of the public landing page.
#
# Deliberately NOT wrapped in `if not st.session_state.get("authentication_status")`,
# and deliberately rendered BEFORE the render_login_or_register()/st.stop() call
# below. It was wrapped in that condition once, and that made it invisible to every
# signed-in user while looking correct in the file and in review: on a fresh page
# load session_state is empty, so the logo rendered, then render_login_or_register()
# read the auth cookie and called st.rerun() (auth.py) - and on that second run the
# condition was False, the element was never emitted, and Streamlit removed it. It
# was then moved after the st.stop() call, which fixed that but hid the logo from
# every signed-out visitor instead, since render_login_or_register() returns False
# and the caller is told to st.stop() before reaching this point (auth.py:263-267).
# "Public landing page" means the product's own front page rather than the admin
# console; it does not mean "logged-out only" or "logged-in only".
st.markdown(
