# Making the SDLC Agents Smarter — Design Plan

Status: proposal for review (no code changed yet)
Author: drafted with Claude Code, 2026-07-13
Scope: the CrewAI SDLC pipeline in `crewai-starter` (agents in
`sdlc/config/agents.yaml`, wired in `sdlc/SDLC.py`).
...
A per-agent markdown file: `sdlc/lessons/<agent_key>.md`.
...
A small helper in `SDLC.py` reads `sdlc/lessons/<agent_key>.md` at crew-build
...
|---|---|---|
| 1 | `sdlc/lessons/` + files seeded from existing backstory incidents | none — pure refactor, behavior-preserving |
| 2 | Loader in `SDLC.py`; slim the backstories | low — verify prompts still assemble correctly |
