...
Per-agent, git-tracked "lessons learned" for the Req2Prod pipeline agents. Each
...
in `req2prod/config/agents.yaml`.
...
the backstories. Purely additive — `agents.yaml` and `Req2Prod.py` are unchanged, so
runtime behavior is identical. The agents do **not** read these files yet.

**Phase 2 (next):** a loader in `Req2Prod.py` appends `lessons/<agent_key>.md` to
