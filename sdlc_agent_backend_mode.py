...
.github/workflows/sdlc-pipeline.yml's resolve_backend job (and
devops-agent.yml's own copy) checks: "api" (default) means every SDLC
...
Marco's own Claude subscription (see sdlc/backend.py) - which only works
...
Same GITHUB_VARIABLES_TOKEN REST pattern as sdlc_deploy_mode.py, for the
...
Deliberately a separate module from sdlc_deploy_mode.py rather than a
...
environment (see streamlit_app.py's "AI Models" tab, and sdlc/backend.py's
